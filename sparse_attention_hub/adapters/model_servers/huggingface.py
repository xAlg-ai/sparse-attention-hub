...
from ..utils.exceptions import ModelCreationError, ResourceCleanupError, TokenizerCreationError
...
class ModelServerHF(ModelServer):
    ...
    def _create_model(self, model_name: str, gpu_id: Optional[int], model_kwargs: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            self.logger.debug(f"Loading HuggingFace model: {model_name} with kwargs: {model_kwargs}")
            ...
            if gpu_id is not None:
                if torch.cuda.is_available():
                    ...
                    self.logger.warning(f"CUDA not available, placing model {model_name} on CPU instead of GPU {gpu_id}")
                    model = model.to("cpu")
            else:
                # Explicitly place on CPU
                model = model.to("cpu")
            
            self.logger.info(f"Successfully created HuggingFace model: {model_name} on {'GPU' if gpu_id is not None else 'CPU'}")
            return model
    ...
    def _create_tokenizer(self, tokenizer_name: str, tokenizer_kwargs: Dict[str, Any]) -> Any:
        ...
        """
        try:
            self.logger.debug(f"Loading HuggingFace tokenizer: {tokenizer_name} with kwargs: {tokenizer_kwargs}")
            ...
            if tokenizer.pad_token is None:
                ...
                self.logger.debug(f"Set pad_token to eos_token for tokenizer: {tokenizer_name}")
            
            self.logger.info(f"Successfully created HuggingFace tokenizer: {tokenizer_name}")
            return tokenizer
            
        except Exception as e:
            self.logger.error(f"Failed to create HuggingFace tokenizer {tokenizer_name}: {e}")
    ...
    def _delete_model(self, model: Any, gpu_id: Optional[int]) -> None:
        ...
        """
        try:
            self.logger.debug(f"Deleting HuggingFace model on {'GPU' if gpu_id is not None else 'CPU'}")
            ...
            if gpu_id is not None and torch.cuda.is_available():
                try:
                    ...
                    self.logger.warning(f"Failed to move model to CPU before deletion: {e}")
            ...
            if gpu_id is not None and torch.cuda.is_available():
                try:
                    ...
                    self.logger.warning(f"Failed to cleanup GPU memory for GPU {gpu_id}: {e}")
            
            self.logger.debug("Successfully deleted HuggingFace model and cleaned up resources")
    ...
    def get_model_with_device_info(self, model_name: str, gpu: Optional[int], model_kwargs: Dict[str, Any]) -> Tuple[Any, str]:
