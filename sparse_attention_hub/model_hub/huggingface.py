...
class ModelHubHF(ModelHub):
    ...
    def addPreAttentionHooks(
        ...
        if not hasattr(model, 'named_modules'):
            raise ValueError("Model must have 'named_modules' method (expected HuggingFace model)")
    ...
    def replaceAttentionInterface(
            ...
            for name, module in model.named_modules():
                ...
                if self._is_attention_module(name, module):
                    if not hasattr(module, 'forward'):
                        raise AttributeError(f"Attention module '{name}' does not have 'forward' method")
        ...
        if attention_modules_found == 0:
            raise ValueError("No attention modules found in model. Expected modules with 'attention' in name and 'forward' method.")
    ...
    def _is_attention_module(self, name: str, module: Any) -> bool:
        ...
        has_attention_name = any(pattern in name_lower for pattern in attention_patterns)
    ...
    def _cleanup_partial_replacement(self, attention_interface_name: str) -> None:
        ...
        if attention_interface_name in self._original_attention_interfaces:
            ...
            for name, original_forward in self._original_attention_interfaces[attention_interface_name].items():
                ...
                for module_name, original_forward in modules.items():
                    if module_name in module_dict:
                        try:
                            ...
                            errors.append(f"Failed to revert module '{module_name}': {e}")
                    else:
                        errors.append(f"Module '{module_name}' not found in model during reversion")
