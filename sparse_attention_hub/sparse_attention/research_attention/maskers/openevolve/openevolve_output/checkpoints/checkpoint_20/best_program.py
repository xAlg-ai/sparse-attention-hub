...
class OpenEvolveMasker(ResearchMasker):
    ...
    def add_mask(
        ...
        **kwargs: Dict[str, Any],
        ...
        # More aggressive sparsification while maintaining quality:
        # 1. Stronger reduction of statistical budget
        budget = torch.ceil(budget.float() * 0.54).long()
        # 2. Slightly larger minimum base to keep variance estimation robust
        min_base = max(num_base_samples, 4)
        budget = torch.clamp(budget, min=min_base)
        # 3. Tighter hard upper cap: at most 8% of keys per row
        max_budget = max(min_base, int(0.08 * sampling_range))
        budget = torch.clamp(budget, max=max_budget)
        # 4. Lower-bound at 2 for extreme short regions
        budget = torch.clamp(budget, min=2)
        # 5. If static denominator is very large, lower sampling even more for ultra-sparse attention
        if torch.median(static_denominator) > 5.0:
            budget = torch.clamp(budget, max=max(min_base, int(0.06 * sampling_range)))
        # 6. Probabilities for mask
