...
class OpenEvolveMasker(ResearchMasker):
    ...
    def add_mask(
        ...
        **kwargs: Dict[str, Any],
        ...
        # ---------------- Hybrid Top-mass + Tail: more adaptive, more aggressive ------------------- #
        # 1. Deterministic: minimum set of keys per row to cover ≥98% of attention mass.
        # 2. Aggressive, adaptive statistical tail (only for residual).
        # This shrinks density substantially, especially for highly peaked attention.

        # (1) Top-mass: For each row, select minimal k s.t. cumsum covers ≥98% mass (or at least 1, at most ~7).
        ...
        max_k = torch.clamp(k_98.max(), min=1, max=min(12, seq_len_keys // 7 + 1)).item()
        arange_topk = torch.arange(max_k, device=expwts.device)[None, None, None, :]
        mask_top = arange_topk < k_98
        top_idx = sorted_idx[..., :max_k]
        ...
        # If top-mass covers all keys (or all but one), do not sample tail
