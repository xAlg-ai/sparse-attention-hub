...
class OpenEvolveMasker(ResearchMasker):
    ...
    def add_mask(
        ...
        **kwargs: Dict[str, Any],
        ...
        # (1) Deterministic selection: minimal top-k to cover 97% exp(attn) mass (slightly more aggressive for density)
        ...
        # Safety: at least 1 (ultra-sparse), max at 10 for memory efficiency
        ...
        # (2) Adaptive statistical tail (ultra-aggressive)
        min_base = max(num_base_samples, 2)
        rel_mass_covered = cumsum[..., -1:] / (static_denominator + 1e-8)
        # Lower tail scale if mass >97% is already covered
        ...
        # Ultra strict tail cap: 5% max
        max_budget = max(min_base, int(0.05 * sampling_range))
        budget = torch.clamp(budget, max=max_budget)
        # Ignore tail for very-short rows (where 97% mass covers almost all tokens)
