...
class PQCacheConfig(TopKMaskerConfig):
    ...
    def __post_init__(self) -> None:
        ...
        if self.metric not in ["euclidean", "ip"]:
            raise ValueError(
                f"metric must be 'euclidean' or 'ip', got '{self.metric}'"
            )
...
class PQCache(TopKMasker):
    ...
    def _should_use_full_attention(
        ...
        total_needed: int = heavy_size + self.init_offset + dims.seq_len_queries + 2**self.pq_bits
    ...
    def _perform_kmeans_clustering(
        ...
        centroids = centroids.reshape(bsz, num_heads, n_subvec_per_head, cent_cnt, subvec_d)
        
        # codes: [bsz * num_heads * n_subvec, n_keys]
        #     → [bsz, n_keys, num_heads, n_subvec]
        codes = codes.reshape(bsz, num_heads, n_subvec_per_head, n_keys).permute(0, 3, 1, 2)
    ...
    def _handle_incremental_keys(
        ...
        cached_codebook, new_keys = self._determine_new_keys(keys, sparse_meta_data, layer_idx)
    ...
    def _determine_new_keys(
        ...
        """
        cached_codebook: Optional[torch.Tensor] = sparse_meta_data["pq_codebook"][layer_idx]
    ...
    def _compute_pq_scores(
        ...
        if num_key_value_groups == 1:
            ...
            repeat_centroids: torch.Tensor = centroids[:, :, None, :, :, :].expand(
                bsz, kv_heads, num_key_value_groups, n_subvec, cent_cnt, -1
        ...
        gathered_scores: torch.Tensor = torch.gather(qk_table, dim=4, index=repeat_codebook_exp)
    ...
    def _create_pq_mask(
        ...
        return self._create_mask_from_rowise_indices(
            dims, topk_indices_adjusted, device, previous_mask.dtype,
        )
