...
class ConfigSearchManager:
    ...
    def __init__(self, base_config: dict):
        ...
        self.results_dir = Path(base_config["optimal_configs_dir"])
    ...
    def search_optimal_config(
        ...
        try:
            ...
            def objective(trial_config):
                ...
                score = runner(attention_config, task, model)
...
def run_phase_one(config: dict) -> Dict[str, OptimalConfig]:
    ...
    print(f"Configs saved to: {config['optimal_configs_dir']}")
    print(f"{'='*80}")
...
def get_all_sparse_configs(weight_file: str = None) -> List[Tuple[str, Optional[ResearchAttentionConfig], Optional[List]]]:
    ...
    # ==================== 5% sparsity configs =================
    # Random sampling 5%
    name = get_random_sampling_config_name(0.02, 0.02, 0.01)
    ...
    name = get_adaptive_config_name(0.001, 0.001, 0.02, 0.01, 0.1, 0.1)
    config = ResearchAttentionConfig(masker_configs=[
    ...
    if weight_file:
        name = get_adaptive_hat_config_name(0.01, 0.01, 0.02, 0.01, 0.25, 0.25)
        config = ResearchAttentionConfig(masker_configs=[
    ...
    # HAT top k 5%
        name = get_hashattention_config_name(0.005, 0.005, 0.04)
        config = ResearchAttentionConfig(masker_configs=[
    ...
    # Oracle top p 5%
    name = get_oracle_top_p_config_name(0.001, 0.001, 0.75)
    config = ResearchAttentionConfig(masker_configs=[
        SinkMaskerConfig(sink_size=0.001),
        LocalMaskerConfig(window_size=0.001),
    ...
    # Oracle top k 5%
    name = get_oracle_top_k_config_name(0.005, 0.005, 0.04)
    config = ResearchAttentionConfig(masker_configs=[
        SinkMaskerConfig(sink_size=0.005),
        LocalMaskerConfig(window_size=0.005),
    ...
    # ==================== 10% sparsity configs =================
    # Random sampling 10%
    name = get_random_sampling_config_name(0.001, 0.001, 0.1)
    config = ResearchAttentionConfig(masker_configs=[
        SinkMaskerConfig(sink_size=0.001),
        LocalMaskerConfig(window_size=0.001),
    ...
    name = get_adaptive_config_name(0.001, 0.001, 0.05, 0.05, 0.25, 0.25)
    config = ResearchAttentionConfig(masker_configs=[
        SinkMaskerConfig(sink_size=0.001),
        LocalMaskerConfig(window_size=0.001),
    ...
    # More configs for 10% and 20% would be added here following the same pattern...
    # For brevity, adding just a few more examples
    
    # Add MagicPig config as example
    configs.append(("magic_pig_example", 
        ResearchAttentionConfig(masker_configs=[
            SinkMaskerConfig(sink_size=32),
            LocalMaskerConfig(window_size=256),
            MagicPigConfig(lsh_l=12, lsh_k=8, center=True)
        ]),
...
def main():
    ...
    try:
        ...
        return
            ...
            print(f"Loaded {len(optimal_configs)} configurations")
