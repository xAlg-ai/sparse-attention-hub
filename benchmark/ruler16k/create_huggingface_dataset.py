...
def get_dataframe(path):
    ...
    """
    df_list = []
    for task_path in Path(path).glob("validation.jsonl"):
...
if __name__ == "__main__":
    data_dir = Path("/workspace/sparse-attention-hub/RULER.apd10/scripts/data/data/16384")  # output of the generate.sh script
    repo_id = "xAlg-AI/att-hub-ruler-16k"
    ...
    for path in data_dir.glob("*/"):
        ...
        #print (f" dataset.push_to_hub(repo_id={repo_id}, config_name={split}, split={split}) || dataset.len() = {len(dataset)}")
        dataset.push_to_hub(repo_id=repo_id, config_name=split, split=split)
