...
class Ruler16K(Benchmark):
    ...
    benchmark_name: str = "ruler16k"
    huggingface_dataset_id: str = "xAlg-AI/att-hub-ruler-16k"
    ...
    def _load_datasets(self) -> pd.DataFrame:
        ...
        for subset in self.subsets_to_run:
            try:
                ...
                subset_dataset = load_dataset(
                    self.huggingface_dataset_id, subset, split=subset
                )
                subset_df = subset_dataset.to_pandas()
                # Add context length as a column for analysis
                subset_df["context_length"] = 16384
                dfs.append(subset_df)
                print(
                    f"  ✓ Loaded {len(subset_df)} samples from {subset} || context_length = 16384"
                )
        ...
        print(
            f"Combined {len(combined_df)} total samples from {len(dfs)} subsets || context_length = 16384"
        )
        return combined_df
