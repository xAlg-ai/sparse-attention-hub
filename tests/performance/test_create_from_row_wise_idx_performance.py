...
class TestCreateFromRowWiseIdxPerformance:
    ...
    def test_create_from_row_wise_idx_performance(self):
        ...
        for shape, k, sparsity, desc in test_configs:
            ...
            data: torch.Tensor = torch.rand(
                batch_dims + (k,), dtype=torch.float32, device=device
            )
            ...
            for mask_type in ["dense", "index"]:
                ...
                print(
                    f"  mask_type={mask_type:6s} | Sparse: {avg_sparse:8.3f} ms | "
        ...
        for category_name, category_results in categories.items():
            ...
            sparse_wins: int = sum(
                1 for r in category_results if r["winner"] == "Sparse"
            )
            avg_speedup: float = sum(r["speedup"] for r in category_results) / len(
                category_results
            )
            ...
            for r in category_results:
                ...
                print(
                    f"  {r['config']:<35s} | Speedup: {r['speedup']:5.2f}x | Winner: {r['winner']} {winner_symbol}"
                )

            print("\n  Summary:")
            print(
                f"    Dense wins: {dense_wins}/{len(category_results)} ({100*dense_wins/len(category_results):.0f}%)"
            )
            print(
                f"    Sparse wins: {sparse_wins}/{len(category_results)} ({100*sparse_wins/len(category_results):.0f}%)"
            )
            print(f"    Average speedup (sparse/dense): {avg_speedup:.2f}x")

            if dense_wins > sparse_wins:
                print("    🏆 Category Winner: Dense mode")
            else:
                print("    ⚡ Category Winner: Sparse mode")
        ...
        print(
            f"  Dense wins: {total_dense_wins}/{len(results)} ({100*total_dense_wins/len(results):.1f}%)"
        )
        print(
            f"  Sparse wins: {total_sparse_wins}/{len(results)} ({100*total_sparse_wins/len(results):.1f}%)"
        )
        ...
        if total_dense_wins > total_sparse_wins:
            ...
            print("   Recommendation: Use mode='dense' as default")
        else:
            print("\n🏆 Overall Winner: Sparse mode")
            print("   Recommendation: Keep mode='sparse' as default")
    ...
    def test_create_from_row_wise_idx_detailed_analysis(self):
        ...
        print("-" * 90)
        print(
            f"{'Sparsity':<10s} | {'k (tokens)':<12s} | {'Sparse (ms)':<12s} | {'Dense (ms)':<12s} | {'Speedup':<8s} | {'Winner':<8s}"
        )
        print("-" * 90)
        ...
        for k, sparsity_str in sparsity_levels:
            ...
            data: torch.Tensor = torch.rand(
                shape[0], k, dtype=torch.float32, device=device
            )
            ...
            print(
                f"{sparsity_str:<10s} | {k:<12d} | {avg_sparse:<12.3f} | {avg_dense:<12.3f} | {speedup:<8.2f} | {winner:<8s}"
            )
