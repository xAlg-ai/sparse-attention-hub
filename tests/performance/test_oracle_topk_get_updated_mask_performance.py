...
class TestOracleTopKGetUpdatedMaskPerformance:
    ...
    def _create_test_inputs(
        ...
        torch.Tensor, torch.Tensor, torch.Tensor, Mask, AttentionTensorDimensions
    ]:
        ...
        keys: torch.Tensor = torch.randn(
            batch_size, num_heads_keys, seq_len_keys, head_dim, device=device
        )
        queries: torch.Tensor = torch.randn(
            batch_size, num_heads_queries, seq_len_queries, head_dim, device=device
        )
        ...
        if use_attention_mask:
            attention_mask = torch.zeros(
                batch_size, 1, seq_len_queries, seq_len_keys, device=device
            )
    ...
    def test_performance_small_sequences(self):
        ...
        print(f"\n{'='*70}")
        print(
            f"Long Context (batch={batch_size}, heads={num_heads}, queries={seq_len_queries}, keys={seq_len_keys})"
        )
        print(f"{'='*70}")
        print(
            f"Old implementation: {old_stats['mean']:.4f} ± {old_stats['std']:.4f} ms"
        )
        print(
            f"New implementation: {new_stats['mean']:.4f} ± {new_stats['std']:.4f} ms"
        )
    ...
    def test_performance_medium_sequences(self):
        ...
        print(f"\n{'='*70}")
        print(
            f"Long Context with Attention Mask (batch={batch_size}, heads={num_heads}, queries={seq_len_queries}, keys={seq_len_keys})"
        )
        print(f"{'='*70}")
        print(
            f"Old implementation: {old_stats['mean']:.4f} ± {old_stats['std']:.4f} ms"
    ...
    def test_performance_varying_heavy_size(self):
        ...
        print(f"\nUsing device: {device}")
        print(
            f"Long Context Varying Heavy Size: B={batch_size}, H={num_heads}, Q={seq_len_queries}, CTX={seq_len_keys}"
        )
        ...
        for heavy_size in [0.02, 0.05, 0.1]:
            ...
            print(f"{'='*70}")
            print(
                f"Heavy Size {heavy_size} (batch={batch_size}, heads={num_heads}, queries={seq_len_queries}, keys={seq_len_keys})"
            )
            print(f"{'='*70}")
            print(
                f"Old implementation: {old_stats['mean']:.4f} ± {old_stats['std']:.4f} ms"
    ...
    def test_performance_gqa_scenario(self):
        ...
        print(f"\nUsing device: {device}")
        print(
            f"GQA Long Context: B={batch_size}, Q_heads={num_heads_queries}, KV_heads={num_heads_keys}, Q={seq_len_queries}, CTX={seq_len_keys}"
        )
        ...
        print(f"\n{'='*70}")
        print(
            f"GQA Scenario (batch={batch_size}, q_heads={num_heads_queries}, kv_heads={num_heads_keys}, queries={seq_len_queries}, keys={seq_len_keys})"
        )
        print(f"{'='*70}")
        print(
            f"Old implementation: {old_stats['mean']:.4f} ± {old_stats['std']:.4f} ms"
    ...
    def test_performance_with_partial_mask(self):
        ...
        print(f"\nUsing device: {device}")
        print(
            f"Long Context with Partial Mask: B={batch_size}, H={num_heads}, Q={seq_len_queries}, CTX={seq_len_keys}"
        )
        ...
        print(f"\n{'='*70}")
        print(
            f"Partial Mask (batch={batch_size}, heads={num_heads}, queries={seq_len_queries}, keys={seq_len_keys})"
        )
        print(f"{'='*70}")
        print(
            f"Old implementation: {old_stats['mean']:.4f} ± {old_stats['std']:.4f} ms"
    ...
    def test_performance_comprehensive_comparison(self):
        ...
        print(
            f"Long Context: B={batch_size}, H={num_heads}, Q={seq_len_queries}, CTX={seq_len_keys}"
        )
        print(f"{'=' * 90}")
        ...
        for name, heavy_size, mask_pattern in test_configs:
            ...
            print(
                f"{name:<20} {old_stats['mean']:>10.4f}     {new_stats['mean']:>10.4f}     {speedup:>6.2f}x"
            )

        print(f"{'-' * 90}")
        overall_speedup: float = total_old_time / total_new_time
        print(
            f"{'OVERALL':<20} {total_old_time:>10.4f}     {total_new_time:>10.4f}     {overall_speedup:>6.2f}x"
        )
        print(f"{'=' * 90}\n")
        ...
        if overall_speedup > 1.1:
            print(
                f"✓ RECOMMENDATION: Use 'new' mode as default (avg {overall_speedup:.2f}x faster)"
            )
        elif overall_speedup < 0.9:
            print(
                f"✓ RECOMMENDATION: Use 'old' mode as default (avg {1/overall_speedup:.2f}x faster)"
            )
        else:
            print(
                "✓ RECOMMENDATION: Performance similar, choose 'new' for cleaner implementation"
            )
