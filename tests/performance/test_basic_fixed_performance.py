...
class TestLocalMaskerPerformance:
    ...
    def test_local_masker_performance(self):
        ...
        for window_size, desc in test_configs:
            ...
            if local_masker._should_use_full_attention(
                tensor_dims, effective_window_size
            ):
                print("Skipping - would use full attention")
                continue
            ...
            for mode_name, mode in [
                ...
                for _ in range(num_iterations):
                    ...
                    _: Mask = local_masker.get_updated_mask(
                        tensor_dims, effective_window_size, keys, empty_mask, mode=mode
                    )
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                ...
                print(
                    f"{mode_name:20s}: avg={avg_time*1000:7.3f}ms, "
...
class TestSinkMaskerPerformance:
    ...
    def test_sink_masker_performance(self):
        ...
        for sink_size, desc in test_configs:
            ...
            keys: torch.Tensor = torch.randn(
                batch_size, num_heads, seq_len_keys, head_dim, device=device
            )
            queries: torch.Tensor = torch.randn(
                batch_size, num_heads, seq_len_queries, head_dim, device=device
            )
            ...
            for mode_name, mode in [("Sparse", "sparse"), ("Dense", "dense")]:
                ...
                for _ in range(num_iterations):
                    ...
                    start_time: float = time.perf_counter()
                    _: Mask = sink_masker.get_updated_mask(
                        tensor_dims, effective_sink_size, keys, empty_mask, mode=mode
                    )
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
