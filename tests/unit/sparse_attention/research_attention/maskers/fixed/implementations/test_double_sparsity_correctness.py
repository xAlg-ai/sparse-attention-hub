...
:license: Apache 2.0
:date: 2025-01-27
:summary: Correctness tests comparing new DoubleSparsityTopKMasker with original implementation.
"""
...
def channel_data() -> Dict[str, List[List[int]]]:
    ...
    config_path = os.path.join(_doublesparse_path, "config", "meta-llama", "Llama-3.1-8B-Instruct.json")
...
@pytest.fixture
def original_attention(test_config: LlamaConfig, channel_data: Dict[str, List[List[int]]], test_params: Dict[str, int]) -> nn.Module:
...
@pytest.fixture
def new_attention(test_config: LlamaConfig, channel_file: str, test_params: Dict[str, int]) -> nn.Module:
    ...
    custom_name = f"double_sparsity_top_k_masker_{heavy_const}_{group_factor}_{label_bits}"
...
def hidden_states(test_config: LlamaConfig, batch_size: int, small_sequence_length: int) -> torch.Tensor:
    ...
    return torch.randn(batch_size, small_sequence_length, test_config.hidden_size, generator=generator)
...
def past_key_value(test_config: LlamaConfig, batch_size: int, past_sequence_length: int) -> Optional[Cache]:
    ...
    past_key_states = torch.randn(batch_size, test_config.num_key_value_heads, past_sequence_length, head_dim, generator=generator)
    generator = torch.Generator().manual_seed(3)
    past_value_states = torch.randn(batch_size, test_config.num_key_value_heads, past_sequence_length, head_dim, generator=generator)
    ...
    cache.update(past_key_states, past_value_states, layer_idx=4, cache_kwargs=cache_kwargs)
    
    return cache


@pytest.fixture
def attention_mask(batch_size: int, small_sequence_length: int, past_sequence_length: int) -> Optional[torch.Tensor]:
    ...
    boolean_mask = torch.tril(torch.ones(small_sequence_length, total_seq_len, dtype=torch.bool), diagonal=total_seq_len - small_sequence_length)
    attention_mask = torch.zeros(small_sequence_length, total_seq_len, dtype=torch.float16)
    attention_mask = attention_mask.masked_fill(boolean_mask == False, float('-inf')).view(1, 1, small_sequence_length, total_seq_len)
    return  None



@pytest.fixture
def position_ids(small_sequence_length: int, past_sequence_length: int) -> Optional[torch.LongTensor]:
...
@pytest.fixture
def position_embeddings(test_config: LlamaConfig, position_ids: torch.LongTensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    head_dim = test_config.hidden_size // test_config.num_attention_heads
    dummy_value_states = torch.randn(1, test_config.num_attention_heads, position_ids.shape[1], head_dim)
...
class TestDoubleSparsityCorrectness:
    ...
    def test_compare_outputs(self, original_attention: nn.Module, new_attention: nn.Module, 
                       hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor],
                       position_ids: Optional[torch.LongTensor], position_embeddings: Tuple[torch.Tensor, torch.Tensor],
                       past_key_value: Optional[Cache], tolerance: float) -> bool:
        ...
        for key in original_attention_dict:
            if key in new_dict:
                assert torch.allclose(original_attention_dict[key], new_dict[key], atol=tolerance)
            else:
                raise ValueError(f"Key {key} not found in new attention")
            assert torch.allclose(original_attention_dict[key], new_dict[key], atol=tolerance)
        print("Shapes", hidden_states.shape, position_ids.shape, position_embeddings[0].shape, position_embeddings[1].shape, past_key_value.get_seq_length())
        print("params", original_attention.label_bits, original_attention.group_factor, original_attention.heavy_const)
        with torch.no_grad():
            ...
            print("Settings", past_key_value.layers[4].get_seq_length(), original_attention.group_factor, original_attention.label_bits, original_attention.heavy_const)
            assert torch.allclose(orig_attention_output, new_attention_output, atol=tolerance)
            assert torch.allclose(orig_attention_weights, new_attention_weights, atol=tolerance)
