...
class ConcreteModelServer(ModelServer):
    ...
    def _create_model(
        ...
        if model_name.endswith("fail") and "delete_fail" not in model_name:
...
class TestModelEntry:
    ...
    def test_model_entry_creation(self) -> None:
        ...
        entry = ModelEntry(
            model=model, reference_count=1, gpu_id=0, model_kwargs=kwargs
        )
    ...
    def test_model_entry_defaults(self) -> None:
        ...
        entry = ModelEntry(
            model=model, reference_count=1, gpu_id=None, model_kwargs=kwargs  # CPU
        )
    ...
    def test_model_entry_timestamp_update(self) -> None:
        ...
        model = Mock()
        entry = ModelEntry(model=model, reference_count=1, gpu_id=0, model_kwargs={})
...
class TestTokenizerEntry:
    ...
    def test_tokenizer_entry_creation(self) -> None:
        ...
        entry = TokenizerEntry(
            tokenizer=tokenizer, reference_count=2, tokenizer_kwargs=kwargs
        )
...
class TestModelServerSingleton:
    ...
    def test_singleton_with_different_configs(self) -> None:
        ...
        ConcreteModelServer(config1)

        # Should raise ValueError when trying to create with different config
        with pytest.raises(
            ValueError, match="Trying to create ModelServer with different config"
        ):
            ConcreteModelServer(config2)
...
class TestTokenizerManagement:
    ...
    def test_get_tokenizer_creates_new(self) -> None:
        ...
        tokenizer = self.server.get_tokenizer(
...
class TestMemoryCleanup:
    ...
    def test_release_memory_and_clean_with_active_references(self) -> None:
        ...
        result = self.server.release_memory_and_clean(
            "test-model", 0, kwargs, force_delete=False
        )
    ...
    def test_release_memory_and_clean_force_delete(self) -> None:
        ...
        # Force clean
        result = self.server.release_memory_and_clean(
            "test-model", 0, kwargs, force_delete=True
        )
